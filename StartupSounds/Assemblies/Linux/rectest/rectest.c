...
BOOL InitDevice(int device)
{
	...
	{ // get list of inputs
		...
		gtk_list_store_clear(GTK_LIST_STORE(gtk_combo_box_get_model(list)));
...
void DeviceChanged(GtkComboBox *obj, gpointer data)
{
	...
	int i = gtk_combo_box_get_active(obj); // get the selection
...
int main(int argc, char* argv[])
{
	...
	{ // get list of recording devices
		int c;
		const char *name;
		GtkComboBox *list = GTK_COMBO_BOX(GetWidget("device"));
		BASS_DEVICEINFO di;
		for (c = 0; BASS_RecordGetDeviceInfo(c, &di); c++)
			gtk_combo_box_append_text(list, di.name);
		gtk_combo_box_set_active(list, 0);
